...
- **No decoded per-protocol tables for Sui lending or DEX.** Unlike EVM (`aave_v3_ethereum.pool_evt_borrow`, `dex.trades`) or Solana (`orca_whirlpool_solana.swap`, `dex_solana.trades`), Sui Move events aren't decoded into per-protocol curated tables for most DeFi domains. Dune's curated `lending.*` tables [cover 15 EVM chains](https://docs.dune.com/data-catalog/curated/lending/overview) — Sui isn't in scope. A few narrow Sui curated tables exist (`sui_walrus.*`, `sui_daily.*`, `sui_tvl.*`), and a `dex_sui.trades` table is observable in the data explorer but isn't documented in the official catalog as of May 2026. For lending and most protocol-specific analytics, you query `sui.events` directly and filter by `event_type` string.
