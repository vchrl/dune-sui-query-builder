...
- **No decoded protocol tables.** Unlike EVM (where `aave_v3_ethereum.pool_evt_borrow` exists) or Solana (`orca_whirlpool_solana.swap`), **Dune does NOT decode Sui Move events into per-protocol tables**. You query `sui.events` directly and filter by `event_type` string.
