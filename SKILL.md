...
| Price for a thin token with no oracle feed | `dex_sui.trades` daily VWAP | Same DEX trades the market priced against; see `sui-curated-tables.md` |
| USD from a lending share balance (cToken, supply-index) | Protocol-emitted USD per share | Share is not underlying; derive USD-per-share on the same object, see `protocol-patterns.md` |
| Checking a priced query / serving repeated reads | Verification toolkit + materialized view | Recount, stablecoin cross-check; matview not `query_<id>` |
