...
A [Claude skill](https://www.anthropic.com/news/skills) for building, debugging, and optimizing **DuneSQL** queries against **Sui** blockchain data — with chained Sui RPC and Pyth Hermes patterns that go beyond what indexed tables alone can deliver.
...
The most-cited "Navi Protocol" dashboard on Dune — [mementomori7777, ~50K views, 19 charts](https://dune.com/mementomori7777/navi-protocol-full-dashboard) — is actually querying **Suilend's** event package the entire time. Triple-confirmed against Suilend's SDK, GitHub repo, and DefiLlama page.

That mislabel is why this skill exists. Sui has **no decoded protocol tables on Dune** (unlike EVM and Solana). You query `sui.events` directly and filter by event_type string — and one wrong package hex propagates as a "well-known" dataset for tens of thousands of viewers. To build correctly, you need methodology, not just SQL.

This skill packages that methodology.
...
Sui's data model is fundamentally different from EVM (no decoded tables) and Solana (object-centric state). Tool selection is non-obvious:
...
### As a Claude skill (Claude.ai, Claude Desktop, Claude Code)
...
The skill auto-triggers on prompts mentioning Dune, DuneSQL, Sui queries, Move events, LiveFetch, Navi, Suilend, Pyth, etc. See the full trigger list in [`SKILL.md`](./SKILL.md).

### As reference documentation (no Claude needed)
...
Sui's lack of decoded protocol tables means every analyst rediscovers the same edge cases — binary type handling, the `::coin::COIN` problem, package upgrades that silently truncate history, the double-hex encoding in `prices.hour`. I built this skill while preparing for a Mysten Labs data interview and shipping a comparative Suilend vs Navi dashboard over a weekend. The dashboard was one output; this skill is the compounding asset.

Public because (1) the methodology is genuinely useful for anyone analyzing Sui on Dune and (2) I'd rather the work live somewhere the next person can find it.

Built by [Vincent Charles](https://github.com/vchrl) — freelance blockchain data analytics (Unchain Data; ex-Binance + Morpho Labs data; currently at Orca). Built with [Claude](https://www.anthropic.com/claude) + [Dune MCP](https://docs.dune.com/api-reference/overview/mcp-server).
...
- [Dune Analytics](https://dune.com) — query engine, LiveFetch (`http_post` / `http_get`), the [MCP server](https://docs.dune.com/api-reference/overview/mcp-server) that made the workflow possible
